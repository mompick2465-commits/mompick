...
### 🎨 게시글 작성 UI 개선 (2024)
- **향상된 버튼 디자인**: 그라데이션 배경과 블러 효과를 적용한 현대적인 버튼 디자인
- **인터랙티브 호버 효과**: 버튼 호버 시 스케일 확대 및 그림자 강화 효과
- **직관적인 레이블**: 각 버튼에 "사진", "이모지", "시설" 레이블 추가로 기능 명확화
- **이모지 인라인 삽입**: 텍스트 입력 중간에 이모지를 삽입할 수 있는 기능 추가
  - 커서 위치에 이모지 자동 삽입
  - 이모지 선택 시 자동으로 피커 닫힘
  - 텍스트와 이모지를 자연스럽게 함께 사용 가능
