...
import { Search, Filter, MapPin, Phone, Users, Star, Edit, Upload, X, Image as ImageIcon, Trash2, Eye, EyeOff } from 'lucide-react'
...
interface KindergartenInfo {
  ...
  telno: string
  establish: string
  reviewCount?: number // API에서 받아온 리뷰 개수
  buildingImageCount?: number // 건물 사진 개수
  mealDatesCount?: number // 급식 날짜 개수
  // 추가 정보
  customInfo?: {
    id: string
...
export default function KindergartensPage() {
  ...
  // 선택된 유치원 정보 수정
  const [selectedKindergarten, setSelectedKindergarten] = useState<KindergartenInfo | null>(null)
  ...
  useEffect(() => {
    const loadRegionCodes = async () => {
      try {
        // 메인 앱의 regionCodes를 동적으로 가져오기
        const response = await fetch('/api/kindergartens/regions')
        if (response.ok) {
  ...
  const handleSearchFromCache = async () => {
    if (!selectedSido || !selectedSgg) {
      ...
      try {
        ...
        const response = await fetch(`/api/kindergartens/cache?sido=${encodeURIComponent(selectedSido)}&sgg=${encodeURIComponent(selectedSgg)}`)
        
        if (response.ok) {
          const data = await response.json()
          console.log('📦 캐시 응답 데이터:', data)
          console.log('📦 첫 번째 유치원:', data.kindergartens?.[0])
          
          if (data.source === 'cache') {
            setKindergartens(data.kindergartens || [])
            alert(`✅ 캐시에서 ${data.count}개 유치원을 불러왔습니다.`)
          } else {
  ...
  const handleSearchFromAPI = async () => {
    ...
    try {
      setLoading(true)
        ...
        const response = await fetch(`/api/kindergartens?sido=${encodeURIComponent(selectedSido)}&sgg=${encodeURIComponent(selectedSgg)}`)
        
        if (response.ok) {
          const data = await response.json()
          console.log('📦 API 응답 데이터:', data)
          console.log('📦 첫 번째 유치원 전체:', JSON.stringify(data.kindergartens?.[0], null, 2))
          console.log('📦 buildingImageCount:', data.kindergartens?.[0]?.buildingImageCount)
          console.log('📦 mealDatesCount:', data.kindergartens?.[0]?.mealDatesCount)
          setKindergartens(data.kindergartens || [])
          
          alert(`📡 API에서 ${data.count}개 유치원을 불러와 캐시에 저장했습니다.`)
        } else {
          const errorData = await response.json()
      ...
      console.error('유치원 목록 조회 오류:', error)
      alert('유치원 목록 조회 중 오류가 발생했습니다.')
      setKindergartens([])
    } finally {
      setLoading(false)
    }
  ...
  // 유치원 선택 - 상세 정보 및 커스텀 정보 조회
  const handleEditKindergarten = async (kindergarten: KindergartenInfo) => {
    try {
      setLoading(true)
      
      // 목록 조회 시 이미 모든 정보를 받아왔으므로 별도 API 호출 없이 사용
      // 커스텀 정보와 리뷰만 추가로 조회
      
      // 1. 커스텀 정보 조회
      let customInfo = null
      try {
        const customResponse = await fetch(`/api/kindergartens/custom/${kindergarten.kindercode}`)
      ...
      try {
        const reviewResponse = await fetch(`/api/kindergartens/reviews/${kindergarten.kindercode}`)
      ...
      // 3. 급식 정보 조회 (날짜별)
      try {
        const mealsResponse = await fetch(`/api/kindergartens/meals/${kindergarten.kindercode}`)
        if (mealsResponse.ok) {
      ...
      // 4. 간편신청 정보 조회
      try {
        const applicationResponse = await fetch(`/api/kindergartens/application/${kindergarten.kindercode}`)
  ...
  const handleDrop = (dropIndex: number, imageType: 'building' | 'meal') => {
    ...
    const imageArray = imageType === 'building' ? formData.building_images : formData.meal_images
    ...
    if (imageType === 'building') {
      ...
      setFormData({ ...formData, meal_images: newImages })
    }
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>, imageType: 'building' | 'meal') => {
    ...
    if (!files || files.length === 0 || !selectedKindergarten) return
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        formData.append('kinderCode', selectedKindergarten.kindercode)
        formData.append('imageType', imageType)

        const response = await fetch('/api/kindergartens/upload', {
          method: 'POST',
          body: formData
        })
  ...
  const handleSaveBuildingImages = async () => {
    ...
    try {
      ...
      // 1. 커스텀 정보 테이블에 저장
      const customResponse = await fetch('/api/kindergartens/custom', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          kinder_code: selectedKindergarten.kindercode,
          kinder_name: selectedKindergarten.kindername,
  ...
  const handleSaveMealInfo = async () => {
    ...
    try {
      ...
      // 급식 정보 저장
      const mealsResponse = await fetch('/api/kindergartens/meals', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          kindergarten_code: selectedKindergarten.kindercode,
          meals: mealsArray
        })
      })
  ...
  // 리뷰 삭제 핸들러
  const handleDeleteReview = async (reviewId: string, kinderCode: string) => {
    ...
    try {
      setLoading(true)
      const response = await fetch(`/api/kindergartens/reviews/${kinderCode}/${reviewId}`, {
        method: 'DELETE'
      })
      ...
      if (response.ok) {
        alert('리뷰가 삭제되었습니다.')
        // 리뷰 목록 새로고침
        const reviewResponse = await fetch(`/api/kindergartens/reviews/${kinderCode}`)
        if (reviewResponse.ok) {
          const reviewData = await reviewResponse.json()
          setSelectedKindergarten({
  ...
  // 리뷰 숨김 처리 핸들러
  const handleHideReview = async (reviewId: string, kinderCode: string) => {
  ...
  // 리뷰 숨김 해제 핸들러
  const handleUnhideReview = async (reviewId: string, kinderCode: string) => {
    try {
      setLoading(true)
      const response = await fetch(`/api/kindergartens/reviews/${kinderCode}/${reviewId}`, {
        method: 'PATCH',
  ...
  const handleSaveApplicationInfo = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/kindergartens/application', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const filteredKindergartens = kindergartens
    .filter(kg =>
      kg.kindername.toLowerCase().includes(searchTerm.toLowerCase()) ||
      kg.addr.toLowerCase().includes(searchTerm.toLowerCase())
    )
    .sort((a, b) => {
      switch (sortBy) {
        case 'rating':
        ...
        case 'name':
          return a.kindername.localeCompare(b.kindername, 'ko') // 가나다 순
        default:
          return 0
      }
    })
  ...
  // 리뷰가 가장 많은 유치원 찾기
  const maxReviewCount = Math.max(...filteredKindergartens.map(kg => kg.reviewCount || 0))
  ...
  return (
    <div className="p-6">
      <div className="mb-4">
        <h1 className="text-xl font-bold text-gray-900">유치원 관리</h1>
        <p className="text-sm text-gray-600">지역별 유치원을 검색하고 상세 정보를 추가하세요</p>
      </div>
      ...
      <Card className="border-0 shadow-none">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg">유치원 목록</CardTitle>
            {kindergartens.length > 0 && (
              <span className="text-sm text-gray-600">{kindergartens.length}개 유치원</span>
            )}
          </div>
          {kindergartens.length > 0 && (
        ...
        <CardContent>
          {kindergartens.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredKindergartens.map((kindergarten) => {
                const isTopReviewed = hasReviews && kindergarten.reviewCount === maxReviewCount && maxReviewCount > 0
                ...
                <div 
                  ...
                  onClick={() => handleEditKindergarten(kindergarten)}
                >
                  {/* 상단: 유치원명과 인기 배지 */}
                  <div className="flex items-start justify-between mb-1">
                    <h3 className="text-sm font-semibold text-gray-900">{kindergarten.kindername}</h3>
                    {isTopReviewed && (
                  ...
                  <div className="flex items-center space-x-1.5 text-xs text-gray-500 mb-2">
                    ...
                    <span className="line-clamp-1">{kindergarten.addr}</span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 pt-2 border-t border-gray-200">
                    ...
                    <div className={`flex items-center space-x-1 px-2 py-1 rounded-md text-xs ${
                      ...
                      <span className="font-medium">
                        {kindergarten.averageRating && kindergarten.reviewCount > 0 ? (
                          <>
                            {kindergarten.averageRating} ({kindergarten.reviewCount})
                          </>
                        ) : (
                          `리뷰 ${kindergarten.reviewCount || 0}`
                        )}
                      </span>
                    </div>
                    
                    {/* 건물 사진 배지 */}
                    {(kindergarten.buildingImageCount || 0) > 0 && (
                      <div className="flex items-center space-x-1 px-2 py-1 bg-blue-50 text-blue-700 rounded-md text-xs">
                        <ImageIcon className="h-3 w-3 text-blue-600" />
                        <span className="font-medium">건물 {kindergarten.buildingImageCount}개</span>
                      </div>
                    )}
                    
                    {/* 급식 배지 */}
                    {(kindergarten.mealDatesCount || 0) > 0 && (
                      <div className="flex items-center space-x-1 px-2 py-1 bg-purple-50 text-purple-700 rounded-md text-xs">
                        ...
                        <span className="font-medium">급식 {kindergarten.mealDatesCount}일</span>
                      </div>
                    )}
                    
                    {/* 간편신청 - 월 금액 배지 */}
                    {kindergarten.monthlyPrice && (
                      <div className="px-2 py-1 bg-orange-50 text-orange-700 rounded-md text-xs font-semibold">
                        월 {kindergarten.monthlyPrice}만원
                      </div>
                    )}
                    
                    {/* 간편신청 - 빈자리 배지 */}
                    {kindergarten.availableSlots && (
                      <div className="px-2 py-1 bg-green-100 text-green-700 rounded-md text-xs font-semibold">
                        빈자리 {kindergarten.availableSlots}개
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-2 space-y-1">
                    ...
                    <div className="text-xs text-gray-600">
                      <span className="font-medium">설립:</span> {kindergarten.establish || '-'}
                    </div>

                    <div className="text-[10px] text-gray-500">
                      코드: {kindergarten.kindercode}
                    </div>
                  </div>
                  </div>
                )
          ...
          {kindergartens.length > 0 && filteredKindergartens.length === 0 && (
      ...
      {/* 유치원 상세보기 모달 */}
      {showEditModal && selectedKindergarten && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-3xl w-full p-4 max-h-[90vh] overflow-y-auto">
            {/* 모달 제목 */}
            <div className="mb-3 pb-2 border-b">
              <h2 className="text-base font-bold text-gray-900">유치원 상세보기</h2>
            </div>

            {/* 유치원 기본 정보 */}
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="text-base font-bold">{selectedKindergarten.kindername}</h3>
                <p className="text-xs text-gray-600 mt-0.5">{selectedKindergarten.addr}</p>
                {selectedKindergarten.reviewCount !== undefined && (
                  <div className="flex items-center space-x-2 mt-1.5">
                    <div className="flex items-center space-x-1">
                      <Star className="h-3 w-3 text-yellow-400 fill-current" />
                      <span className="text-xs font-medium">{selectedKindergarten.averageRating || 0}</span>
                    </div>
                    <span className="text-[10px] text-gray-500">리뷰 {selectedKindergarten.reviewCount}개</span>
                  </div>
                )}
              </div>
              <button
            ...
            <div className="flex border-b mb-4">
              ...
              >
                리뷰 ({selectedKindergarten.reviewCount || 0})
              </button>
              </div>
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {(mealData[selectedMealDate]?.images || []).map((url, index) => (
                          <div 
                            ...
                            className="relative group cursor-move"
                            draggable
                            onDragStart={() => handleDragStart(index, 'meal')}
                            onDragOver={handleDragOver}
                            onDrop={() => {
                              if (draggedIndex === null || draggedType !== 'meal') return
              ...
              <div className="space-y-4">
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mb-4">
                  <p className="text-xs text-orange-800">
                    💡 앱의 유치원 간편신청 목록에서 "월 -만원", "빈자리 -개" 형태로 표시됩니다.
                  </p>
                </div>
            ...
            {activeTab === 'reviews' && (
              <div className="space-y-3">
                {selectedKindergarten.reviews && selectedKindergarten.reviews.length > 0 ? (
                  <div className="space-y-2.5">
                    {selectedKindergarten.reviews.map((review) => (
                      <div key={review.id} className="border rounded-lg p-3 bg-gray-50">
                          ...
                          <>
                            ...
                            {review.images && Array.isArray(review.images) && review.images.length > 0 && (
                              <div className="mb-2">
                                ...
                                <div className="mt-1.5 grid grid-cols-3 gap-1.5">
                                  {review.images.map((image: any, index: number) => {
                                    // ReviewImage 타입이면 image_url, 문자열 배열이면 직접 사용
                        ...
                        <div className="flex items-center justify-between pt-2 border-t">
                          ...
                          <div className="flex items-center space-x-1">
                            {review.is_hidden ? (
                              <button
                                onClick={() => handleUnhideReview(review.id, selectedKindergarten.kindercode)}
                              ...
                              <button
                                onClick={() => handleHideReview(review.id, selectedKindergarten.kindercode)}
                            ...
                            <button
                              onClick={() => handleDeleteReview(review.id, selectedKindergarten.kindercode)}
