...
export default function ModalBannersPage() {
  ...
  const fetchBanners = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/banners/main?type=modal')
      if (response.ok) {
  ...
  const uploadImageToStorage = async (file: File): Promise<string | null> => {
    try {
      ...
      const fileName = `modal_${timestamp}_${file.name.replace(/[^a-zA-Z0-9.]/g, '_')}`
      
      const formData = new FormData()
      formData.append('file', file)
      formData.append('fileName', fileName)
      formData.append('bannerType', 'modal')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        alert(editingBanner ? '모달 광고가 수정되었습니다.' : '모달 광고가 추가되었습니다.')
  ...
  return (
    <div className="p-6">
      <div className="mb-4">
        <h1 className="text-xl font-bold text-gray-900">모달 광고 관리</h1>
        <p className="text-sm text-gray-600">로그인 후 하단에 모달로 표시될 광고를 관리하세요</p>
      </div>

      <Card className="border-0 shadow-none">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg">모달 광고 목록</CardTitle>
            <Button
              onClick={() => {
                setEditingBanner(null)
                setFormData({
          ...
          <div className="bg-white rounded-lg max-w-xl w-full p-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-bold">
                {editingBanner ? '모달 광고 수정' : '모달 광고 추가'}
              </h2>
              <button
                onClick={() => {
                  setShowAddModal(false)
              ...
              <div>
                ...
                <div className="mb-2 p-2 bg-blue-50 border border-blue-200 rounded-md">
                  ...
                  <div className="text-[9px] text-blue-700 space-y-0.5">
                    <p>• 최적: <span className="font-semibold">1080 x 360 픽셀</span> (3:1 비율)</p>
                    <p>• 권장: 900 x 300 픽셀 이상</p>
                    <p>• 최소: 720 x 240 픽셀</p>
                  </div>
                </div>
                ...
                {formData.image_url && (
                  <div className="mt-2">
                    <p className="text-[10px] text-gray-600 mb-1">미리보기 (앱 하단 모달 실제 크기 - 화면의 30%):</p>
                    <div className="w-full h-[30vh] bg-gray-100 rounded-t-2xl overflow-hidden border border-gray-300 shadow-lg relative">
                    ...
                    <p className="text-[9px] text-gray-500 mt-1">
                      ※ 앱에서 화면 하단에 이 크기로 표시됩니다 {banners.length > 1 && `(여러 광고 중 ${formData.order_index + 1}번째)`}
                    </p>
                    {selectedFile && (
                      <button
                        type="button"
