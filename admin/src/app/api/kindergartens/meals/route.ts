...
export async function POST(request: Request) {
  try {
    ...
    const { kindergarten_code, meals } = body

    if (!kindergarten_code || !meals || !Array.isArray(meals)) {
      return NextResponse.json({
        error: '유치원 코드와 급식 정보가 필요합니다.'
      }, { status: 400 })
    }

    console.log(`📅 급식 정보 저장 - ${kindergarten_code}: ${meals.length}개 날짜`)
      ...
      meals.map(async (meal) => {
        const { data, error } = await supabase
          ...
          .upsert({
            ...
            onConflict: 'kindergarten_code,meal_date'
          })
          .select()
