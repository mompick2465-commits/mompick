...
export async function GET(request: Request) {
  try {
    ...
    console.log(`유치원 조회 - ${sido} (${sidoCode}) > ${sgg} (${sggCode})`)

    // 관리자 페이지는 무조건 API 호출 (최신 데이터 보장)
    ...
    const apiUrl = `https://e-childschoolinfo.moe.go.kr/api/notice/basicInfo2.do?key=${apiKey}&sidoCode=${sidoCode}&sggCode=${sggCode}`
    ...
    console.log(`📡 API 응답: ${apiData.kinderInfo?.length || 0}개 유치원`)
    console.log('📡 API 상태:', apiData.status)
    ...
    if (!supabaseUrl || !supabaseServiceKey) {
      return NextResponse.json({
        kindergartens: apiData.kinderInfo || [],
        count: apiData.kinderInfo?.length || 0,
        source: 'api',
    ...
    const kindergartensWithReviewCount = await Promise.all(
      (apiData.kinderInfo || []).map(async (kindergarten: any) => {
        // 리뷰 개수 및 평균 별점
        ...
        if (reviewError) {
          console.error(`❌ 리뷰 조회 오류 (${kindergarten.kindername}):`, reviewError)
        }
    ...
    // 캐시 저장 (Edge Function과 동일한 형식)
    try {
      const isoDate = new Date().toISOString().split('T')[0]
      const pathPrefix = `regions/${sidoCode}/${sggCode}`
      
      // Edge Function 형식의 캐시 Envelope 생성
      const cacheEnvelope = {
        meta: {
          sido,
          sgg,
          ...
          itemCount: kindergartensWithReviewCount.length,
          apiVersion: '1.0'
        },
        data: kindergartensWithReviewCount
      }
      ...
      console.log(`📸 스냅샷 저장 시작: kindergarten-cache/${snapshotPath}`)
      ...
      if (snapshotError) {
        ...
        console.log(`✅ 스냅샷 저장 성공: kindergarten-cache/${snapshotPath}`)
      }

      // 2. latest.json 저장
      const latestPath = `${pathPrefix}/latest.json`
      console.log(`🔄 latest.json 저장 시작: kindergarten-cache/${latestPath}`)
      ...
      if (latestError) {
        ...
        console.log(`✅ latest.json 저장 성공: kindergarten-cache/${latestPath}`)
      }

      console.log(`💾 캐시 저장 완료: ${sido}/${sgg} (${kindergartensWithReviewCount.length}개 유치원)`)
    } catch (cacheError) {
      console.error('캐시 저장 중 오류:', cacheError)
    }
