...
export async function POST(request: Request) {
  try {
    ...
    if (!kinder_code || !kinder_name) {
      return NextResponse.json({
        error: '유치원 코드와 이름이 필요합니다.'
      }, { status: 400 })
    }

    console.log(`커스텀 정보 저장 - ${kinder_name} (${kinder_code})`)
    ...
    // kindergarten_custom_info 테이블에 저장 (upsert)
    ...
    console.log(`✅ 커스텀 정보 저장 성공: ${kinder_code}`)
