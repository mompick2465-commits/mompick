...
export async function GET(request: Request) {
  try {
    ...
    console.log(`어린이집 캐시 조회 - ${sido} (${sidoCode}) > ${sgg} (${sggCode})`)
    ...
    // 캐시 경로: childcare-cache/regions/{sidoCode}/{sggCode}/latest.json
    const pathPrefix = `regions/${sidoCode}/${sggCode}`

    console.log('📂 어린이집 캐시 경로:', pathPrefix)
    ...
    try {
      ...
      if (listError || !files || files.length === 0) {
        console.log('❌ 어린이집 캐시 없음:', pathPrefix)
        return NextResponse.json({
      ...
      if (!jsonFile) {
        console.log('❌ JSON 파일 없음:', pathPrefix)
        return NextResponse.json({
          error: '해당 지역의 캐시가 없습니다.',
          source: 'none',
          childcareCenters: [],
      ...
      const filePath = `${pathPrefix}/${jsonFile.name}`
      ...
      if (fileError || !fileData) {
        console.log('❌ 파일 다운로드 실패:', fileError)
        return NextResponse.json({
          error: '캐시 파일을 읽을 수 없습니다.',
          source: 'none',
          childcareCenters: [],
          count: 0
        }, { status: 500 })
      ...
      // 캐시 구조: { meta: {...}, data: [...] }
      const childcareList = fileJson.data || []
      
      console.log(`✅ 어린이집 캐시에서 로드: ${childcareList.length}개`)
      ...
      console.error('어린이집 캐시 로드 오류:', error)
      return NextResponse.json({
