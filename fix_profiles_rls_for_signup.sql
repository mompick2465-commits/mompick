-- profiles 테이블 RLS 정책 수정 (회원가입 프로세스 지원)
...
DROP POLICY IF EXISTS "Allow users to read profiles for notifications" ON profiles;
...
-- SELECT 정책 (인증된 사용자는 모든 프로필 조회 가능 - 회원가입 프로세스 지원)
